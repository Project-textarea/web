...
export default {
    ...
    checkFirstSite: function () {
        // console.log("sessionStorage.getItem('firstUser')", sessionStorage.getItem('firstUser'))
        if (sessionStorage.getItem('firstUser') == null) {
            sessionStorage.setItem('firstUser', true);
            setTimeout(() => {
                location.reload()
            }, 1000)
            // window.open('http://192.168.3.101:8888/#/dismantling')
        }
    },
    isMobile: function () {
    ...
    async checkChainId() {
        ...
        let chainId = null;
        if (!window.ethereum) {
            await initContracts().web3.eth.getChainId().then((id => {
                chainId = id
            }))
            if (chainId == '0x4' || chainId == '4') {
                return true;
            } else {
                return false;
            }
...
export function generalImage(sentence, sentenceColor, creator) {
    ...
    svg += 'rgb(' + r + ',' + g + ',' + b + ');';
    svg += 'font-family:Unifont;font-size:22px;text-anchor:start}</style>'
    ...
    for (; i < line; ++i) {
        ...
        svg += '<text x="20" y="' + (startY + i * 30) + '" class="base">' + sentence.substring(start, end) + '</text>'
    }
    //let simpleCreator = creator.substring(0, 6) + "...." + creator.substring(creator.length - 4);
    //svg += '<text x="185" y="' + (startY + i * 30 + 5) + '" class="creator">Bind by:' + simpleCreator + '</text>';
    svg += '<text x="68" y="' + (startY + (i - 1) * 30 + 32) + '" class="creator">by:' + creator + '</text>'
    svg += '</svg>';
